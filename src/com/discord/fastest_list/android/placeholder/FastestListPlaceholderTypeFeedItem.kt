...
internal class FastestListPlaceholderTypeFeedItem(config: FeedItem) : FastestListPlaceholder() {
   ...
   private class FeedItemDrawable(config: FeedItem, labelWidthRatio: Float, labelWidthRatioSecondary: Float) : CustomDrawDrawable {
      ...
      public open fun draw(canvas: Canvas) {
         ...
         val var6: Float = 2;
         val var5: Float = var12 / 2;
         val var21: Float = var20 / 2;
         val var19: Float = this.config.getLabelPaddingInnerRatio() * var12 / var6;
         val var8: Float = this.config.getLabelPaddingInnerRatio() * var20 / var6;
         val var7: Float = this.config.getPadding() + this.config.getShapeSize() + this.config.getLabelPadding();
         val var13: Float = var15;
         val var9: Float = var15 - this.config.getPadding() - this.config.getLabelPadding();
         val var11: Float = var16;
         val var23: Float = (var16 - (var12 + var20)) / 2.0F + var12;
         var1.drawRoundRect(
            var7,
            ((float)var16 - (var12 + var20)) / 2.0F + var19,
            this.labelWidthRatio * var9,
            ((float)var16 - (var12 + var20)) / 2.0F + var12 - var19,
            var5,
            var5,
            this.paint
         );
         var1.drawRoundRect(var7, var23 + var8, var9 * this.labelWidthRatioSecondary, var23 + var20 - var8, var21, var21, this.paint);
         if (this.config.getDivider()) {
            this.paint.setColor(this.config.getDividerColor());
            var1.drawRect(
               this.config.getDividerPaddingLeft(), var11 - (float)SizeUtilsKt.getDpToPx(1), var13 - this.config.getDividerPaddingRight(), var11, this.paint
            );
            this.paint.setColor(this.config.getColor());
         }
      }
