   ...
   private class FeedItemDrawable(config: FeedItem, labelWidthRatio: Float, labelWidthRatioSecondary: Float) : CustomDrawDrawable {
      ...
      public override fun draw(canvas: Canvas) {
         ...
         if (var14 != 1) {
            ...
            val var2: Float = (var15 - this.config.getShapeSize()) / 2.0F;
         ...
         val var7: Float = this.config.getLabelSize();
         val var21: Float = this.config.getLabelSecondarySize();
         var var6: Float = 2;
         val var19: Float = var7 / 2;
         val var20: Float = var21 / 2;
         val var5: Float = this.config.getLabelPaddingInnerRatio() * var7 / var6;
         val var9: Float = this.config.getLabelPaddingInnerRatio() * var21 / var6;
         var6 = this.config.getPadding() + this.config.getShapeSize() + this.config.getLabelPadding();
         val var11: Float = var16;
         val var13: Float = var16 - this.config.getPadding() - this.config.getLabelPadding();
         val var10: Float = var15;
         val var23: Float = (var15 - (var7 + var21)) / 2.0F + var7;
         var1.drawRoundRect(
            var6,
            ((float)var15 - (var7 + var21)) / 2.0F + var5,
            this.labelWidthRatio * var13,
            ((float)var15 - (var7 + var21)) / 2.0F + var7 - var5,
            var19,
            var19,
            this.paint
         );
         var1.drawRoundRect(var6, var23 + var9, var13 * this.labelWidthRatioSecondary, var23 + var21 - var9, var20, var20, this.paint);
         if (this.config.getDivider()) {
            this.paint.setColor(this.config.getDividerColor());
            var1.drawRect(
               this.config.getDividerPaddingLeft(), var10 - (float)SizeUtilsKt.getDpToPx(1), var11 - this.config.getDividerPaddingRight(), var10, this.paint
            );
            this.paint.setColor(this.config.getColor());
         }
      }
