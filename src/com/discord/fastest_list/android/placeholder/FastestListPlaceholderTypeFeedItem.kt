...
internal class FastestListPlaceholderTypeFeedItem(config: FeedItem) : FastestListPlaceholder {
   ...
   private class FeedItemDrawable(config: FeedItem, labelWidthRatio: Float, labelWidthRatioSecondary: Float) : CustomDrawDrawable {
      ...
      public open fun draw(canvas: Canvas) {
         ...
         var var6: Float = 2;
         val var20: Float = var11 / 2;
         val var19: Float = var5 / 2;
         val var21: Float = this.config.getLabelPaddingInnerRatio() * var11 / var6;
         var6 = this.config.getLabelPaddingInnerRatio() * var5 / var6;
         val var9: Float = this.config.getPadding() + this.config.getShapeSize() + this.config.getLabelPadding();
         val var12: Float = var15;
         val var13: Float = var15 - this.config.getPadding() - this.config.getLabelPadding();
         val var7: Float = var16;
         val var23: Float = (var16 - (var11 + var5)) / 2.0F + var11;
         var1.drawRoundRect(
            var9,
            ((float)var16 - (var11 + var5)) / 2.0F + var21,
            this.labelWidthRatio * var13,
            ((float)var16 - (var11 + var5)) / 2.0F + var11 - var21,
            var20,
            var20,
            this.paint
         );
         var1.drawRoundRect(var9, var23 + var6, var13 * this.labelWidthRatioSecondary, var23 + var5 - var6, var19, var19, this.paint);
         if (this.config.getDivider()) {
            this.paint.setColor(this.config.getDividerColor());
            var1.drawRect(
               this.config.getDividerPaddingLeft(), var7 - (float)SizeUtilsKt.getDpToPx(1), var12 - this.config.getDividerPaddingRight(), var7, this.paint
            );
            this.paint.setColor(this.config.getColor());
         }
      }
