   ...
   private class FeedItemDrawable(config: FeedItem, labelWidthRatio: Float, labelWidthRatioSecondary: Float) : CustomDrawDrawable {
      ...
      public override fun draw(canvas: Canvas) {
         ...
         if (this.config.getBackgroundColor() != 0) {
            this.path.reset();
            this.pathRect.set(0.0F, 0.0F, (float)var15, (float)var16);
         ...
         if (var14 != 1) {
            ...
            val var3: Float = (var16 - this.config.getShapeSize()) / 2.0F;
         ...
         var var6: Float = 2;
         val var19: Float = var9 / 2;
         val var20: Float = var21 / 2;
         val var5: Float = this.config.getLabelPaddingInnerRatio() * var9 / var6;
         var6 = this.config.getLabelPaddingInnerRatio() * var21 / var6;
         val var7: Float = this.config.getPadding() + this.config.getShapeSize() + this.config.getLabelPadding();
         val var12: Float = var15;
         val var11: Float = var15 - this.config.getPadding() - this.config.getLabelPadding();
         val var8: Float = var16;
         val var23: Float = (var16 - (var9 + var21)) / 2.0F + var9;
         var1.drawRoundRect(
            var7,
            ((float)var16 - (var9 + var21)) / 2.0F + var5,
            this.labelWidthRatio * var11,
            ((float)var16 - (var9 + var21)) / 2.0F + var9 - var5,
            var19,
            var19,
            this.paint
         );
         var1.drawRoundRect(var7, var23 + var6, var11 * this.labelWidthRatioSecondary, var23 + var21 - var6, var20, var20, this.paint);
         if (this.config.getDivider()) {
            this.paint.setColor(this.config.getDividerColor());
            var1.drawRect(
               this.config.getDividerPaddingLeft(), var8 - (float)SizeUtilsKt.getDpToPx(1), var12 - this.config.getDividerPaddingRight(), var8, this.paint
            );
            this.paint.setColor(this.config.getColor());
         }
      }
