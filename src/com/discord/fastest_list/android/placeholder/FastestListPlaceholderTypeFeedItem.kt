   ...
   private class FeedItemDrawable(config: FeedItem, labelWidthRatio: Float, labelWidthRatioSecondary: Float) : CustomDrawDrawable {
      ...
      public override fun draw(canvas: Canvas) {
         ...
         if (this.config.getBackgroundColor() != 0) {
            this.path.reset();
            this.pathRect.set(0.0F, 0.0F, (float)var16, (float)var15);
         ...
         val var14: Int = FastestListPlaceholderTypeFeedItem.FeedItemDrawable.WhenMappings.$EnumSwitchMapping$0[this.config.getShapeType().ordinal()];
         if (var14 != 1) {
            if (var14 == 2) {
         ...
         val var9: Float = this.config.getLabelSize();
         val var21: Float = this.config.getLabelSecondarySize();
         var var6: Float = 2;
         val var5: Float = var9 / 2;
         val var19: Float = var21 / 2;
         val var20: Float = this.config.getLabelPaddingInnerRatio() * var9 / var6;
         val var8: Float = this.config.getLabelPaddingInnerRatio() * var21 / var6;
         val var13: Float = this.config.getPadding() + this.config.getShapeSize() + this.config.getLabelPadding();
         var6 = var16;
         val var11: Float = var16 - this.config.getPadding() - this.config.getLabelPadding();
         val var10: Float = var15;
         val var23: Float = (var15 - (var9 + var21)) / 2.0F + var9;
         var1.drawRoundRect(
            var13,
            ((float)var15 - (var9 + var21)) / 2.0F + var20,
            this.labelWidthRatio * var11,
            ((float)var15 - (var9 + var21)) / 2.0F + var9 - var20,
            var5,
            var5,
            this.paint
         );
         var1.drawRoundRect(var13, var23 + var8, var11 * this.labelWidthRatioSecondary, var23 + var21 - var8, var19, var19, this.paint);
         if (this.config.getDivider()) {
            this.paint.setColor(this.config.getDividerColor());
            var1.drawRect(
               this.config.getDividerPaddingLeft(), var10 - (float)SizeUtilsKt.getDpToPx(1), var6 - this.config.getDividerPaddingRight(), var10, this.paint
            );
            this.paint.setColor(this.config.getColor());
         }
      }
